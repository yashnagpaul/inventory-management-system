...
function getWarehouseInventorybyId(id) {
  ...
  const warehouseInventory = inventoryList.filter(
...
function searchWarehouses(searchWord) {
  ...
  const returnWarehousesByCity = warehouseList.filter(
    (warehouse) => warehouse.city === searchWord
  );
  const returnWarehousesByName = warehouseList.filter(
    (warehouse) => warehouse.name === searchWord
  );
  const returnWarehousesByAddress = warehouseList.filter(
    (warehouse) => warehouse.address === searchWord
  );
  const returnWarehousesByCountry = warehouseList.filter(
    (warehouse) => warehouse.country === searchWord
  );
  const returnWarehousesByContactName = warehouseList.filter(
    (warehouse) => warehouse.contact.name === searchWord
  );
  const returnWarehousesByContactPosition = warehouseList.filter(
    (warehouse) => warehouse.contact.position === searchWord
  );
  const returnWarehousesByContactPhone = warehouseList.filter(
    (warehouse) => warehouse.contact.phone === searchWord
  );
  const returnWarehousesByContactEmail = warehouseList.filter(
    (warehouse) => warehouse.contact.email === searchWord
  );

  const totalWarehouses = concatWarehouses
    .concat(returnWarehousesByCity)
    .concat(returnWarehousesByName)
    .concat(returnWarehousesByAddress)
    .concat(returnWarehousesByCountry)
    .concat(returnWarehousesByContactName)
    .concat(returnWarehousesByContactPosition)
    .concat(returnWarehousesByContactPhone)
    .concat(returnWarehousesByContactEmail);

  const uniqueWarehouses = Array.from(
    new Set(totalWarehouses.map((a) => a.id))
  ).map((id) => {
    return totalWarehouses.find((a) => a.id === id);
  });
...
function editWarehouse({
  ...
  const thisWarehouseIndex = warehouseList.findIndex(
